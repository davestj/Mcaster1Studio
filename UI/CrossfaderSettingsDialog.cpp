...
namespace {
    const QString kDlgBg      = "#1e2128";
    const QString kPanelBg    = "#252830";
    const QString kBorderCol  = "#3a3f4b";
    const QString kTextPrimary= "#e8eaf0";
    const QString kTextMuted  = "#8890a0";
    const QString kAccentBlue = "#1c5caa";
    ...
    QString comboQss() {
        return QString(
            ...
            "  outline:none;"
            "}"
        ).arg(kPanelBg, kTextPrimary, kBorderCol, kAccentBlue);
    }
    ...
    QString spinQss() {
        ...
        ).arg(kPanelBg, kTextPrimary, kBorderCol);
    }
    ...
    QString checkQss() {
        ...
        ).arg(kTextPrimary, kBorderCol, kPanelBg, kAccentBlue);
    }

    QString btnQss(const QString& bg = "#2a2f3a") {
        return QString(
            "QPushButton {"
            "  background:%1; color:%2; border:1px solid %3; border-radius:3px;"
        ...
        ).arg(bg, kTextPrimary, kBorderCol,
              QColor(bg).lighter(130).name(), kAccentBlue);
    }
}
...
void CrossfadeCurveView::buildChart() {
    auto* c = new QChart;
    c->setBackgroundBrush(QBrush(QColor("#0a0d14")));
    c->setPlotAreaBackgroundBrush(QBrush(QColor("#0a0d14")));
    ...
    m_axisX->setLabelsColor(QColor("#607080"));
    m_axisX->setGridLineColor(QColor("#1a2030"));
    m_axisX->setLinePen(QPen(QColor("#2a3040")));
    m_axisX->setTitleBrush(QBrush(QColor("#607080")));

    m_axisY = new QValueAxis;
    m_axisY->setRange(0, 1.05);
    m_axisY->setTickCount(6);
    m_axisY->setLabelFormat("%.0f%%");
    m_axisY->setLabelsColor(QColor("#607080"));
    m_axisY->setGridLineColor(QColor("#1a2030"));
    m_axisY->setLinePen(QPen(QColor("#2a3040")));
    ...
    for (auto* s : {(QAbstractSeries*)m_outArea, m_inArea, m_threshold,
                    m_outCtrlPts, m_inCtrlPts}) {
        s->attachAxis(m_axisX);
        s->attachAxis(m_axisY);
    }

    setChart(c);
    setStyleSheet("background:#0a0d14; border:1px solid #2a3040;");
}

float CrossfadeCurveView::evalCurve(CrossfadeSettings::CurveType type,
                                     float t, float level, bool fadeOut) const {
    // t is normalized time 0→1 over the fade duration
    switch (type) {
    ...
    case CrossfadeSettings::Custom: {
        // Bezier cubic through stored control points (or fallback to S-Curve)
...
QVector<QPointF> CrossfadeCurveView::calcCurve(bool fadeOut) const {
    ...
    // Fade-in starts at fixedFadeTime or auto-detect offset
...
void CrossfadeCurveView::rebuildSeries() {
    ...
    if (m_s.triggerMode == CrossfadeSettings::AutoDetect) {
        // Convert dB to 0–1 linear (rough approximation for display)
...
void CrossfadeCurveView::mousePressEvent(QMouseEvent* e) {
    ...
    // ── Check if near a custom control point ──────────────────────────────
    ...
    // ── Click on chart in Custom mode → add control point ────────────────
...
void CrossfadeCurveView::mouseMoveEvent(QMouseEvent* e) {
    ...
    // Show crosshair cursor near draggable elements
...
void CrossfaderSettingsDialog::buildUi() {
    ...
    // ── Title ─────────────────────────────────────────────────────────────
    {
        ...
        titleLbl->setStyleSheet(
            "QLabel { color:#c8d0e0; font-size:14px; font-weight:700;"
            "  border-bottom:1px solid #2a3040; padding-bottom:6px;"
    ...
    // ── Top row: Fade Out | Fade In panels ────────────────────────────────
    {
                                  ...
                                  QSlider*&     levelSl, QLabel*& levelLbl) -> QGroupBox*
        {
            ...
            timeSl = makeSlider(5, 300, 50, g);  // 0.5–30.0 s (×10)
        ...
        auto* outGroup = makeFadePanel("Fade Out  (outgoing track)",
            m_outEnable, m_outCurve,
            m_outTimeSl, m_outTimeLbl,
            m_outLevelSl, m_outLevelLbl);

        auto* inGroup = makeFadePanel("Fade In  (incoming track)",
            m_inEnable, m_inCurve,
            m_inTimeSl, m_inTimeLbl,
            m_inLevelSl, m_inLevelLbl);
    ...
    // ── Chart ─────────────────────────────────────────────────────────────
    {
        auto* chartGroup = makeGroup("Crossfade Preview  —  drag the red threshold line • Ctrl+click to add custom points", this);
        ...
        auto* note = new QLabel(
            ...
            "•  Custom curve: set Curve to 'Custom', then Ctrl+click to add control points, drag to reshape",
            chartGroup);
    ...
    // ── Separator ─────────────────────────────────────────────────────────
    {
        ...
        sep->setStyleSheet("QFrame { color:#2a3040; }");
        root->addWidget(sep);
    }
    ...
    // ── Buttons ───────────────────────────────────────────────────────────
    {
        ...
        restoreBtn->setStyleSheet(btnQss("#2a2f3a"));
        connect(restoreBtn, &QPushButton::clicked, this, &CrossfaderSettingsDialog::onRestoreDefaults);

        auto* okBtn     = new QPushButton("OK", this);
        auto* cancelBtn = new QPushButton("Cancel", this);
        okBtn->setStyleSheet(btnQss(kAccentBlue));
        cancelBtn->setStyleSheet(btnQss("#2a2f3a"));
    ...
    for (auto* w : {(QWidget*)m_outEnable, m_outCurve, m_outTimeSl, m_outLevelSl,
    ...
    // Update trigger group visibility
...
void CrossfaderSettingsDialog::onControlChanged() {
    ...
    CrossfadeSettings s = collectSettings();
    m_chart->updateCurves(s);
}
...
void CrossfaderSettingsDialog::onCustomPointMoved(bool fadeOut, int /*idx*/, QPointF /*n*/) {
    CrossfadeSettings s = collectSettings();
    // sync the updated points back from the chart state
    m_chart->updateCurves(s);
}
