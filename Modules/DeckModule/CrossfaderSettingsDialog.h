...
public:
    ...
    void refreshTheme();   ///< Reapply chart colors for current theme
...
private slots:
    ...
    void onThemeChanged();         ///< reapply colors when global theme changes

private:
    void buildUi();
    void applySettings(const CrossfadeSettings& s);
    CrossfadeSettings collectSettings() const;
    void applyDialogColors();      ///< rebuild all QSS using current theme
    void applyChartColors();       ///< update QChart colors for current theme
